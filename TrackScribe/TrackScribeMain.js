...
TSMain = function () {
	...
	this.map = null;
	this.mode = TSEditModes.VIEW;
	this.clickListener = null;
	this.pointList = new TSList();
};

TSMain.prototype.setMode = function (mode) {
	...
	switch (this.mode) {
	case TSEditModes.VIEW:
		break;
	case TSEditModes.ADD_MANUALNODE:
		this.clickListener = google.maps.event.addListener(this.map, 'click', function(mouseEvent) {
			var latLng = mouseEvent.latLng;
			var typeAdded = that.pointList.addNode(latLng,that.map);
		...
		break;
	case TSEditModes.ADD_MANUALPOINT:
...
TSMain.prototype.initialize = function() {
	this.map = new google.maps.Map(document.getElementById("map-canvas"),
			this.mapOptions);
	//this.setMode(TSEditModes.ADD_MANUALNODE);
};
